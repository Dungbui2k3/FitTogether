...
const EditFieldModal: React.FC<EditFieldModalProps> = ({ isOpen, field, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (field) {
      setFormData({
        ...
        phone: field.phone,
        facilities: field.facilities && field.facilities.length > 0 ? [...field.facilities] : [''],
  ...
  const handleInputChange = (fieldName: keyof UpdateFieldRequest, value: string) => {
    setFormData(prev => ({
      ...prev,
      [fieldName]: value,
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fieldService.updateField(field.id, formDataToSend as any);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Chỉnh Sửa Sân Thể Thao</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* Upload New Images */}
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-2">Thêm hình ảnh mới</h4>
